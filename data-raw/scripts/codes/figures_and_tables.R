...
tab_main_specification <- function(res_list = list.files("results/estimations/", pattern = "_optimal.rds", full.names = TRUE)) {
  res <- as.data.frame(
      ...
      lapply(
        ...
        function(file) {
          tryCatch({
            ...
            res <- res[res$sample %in% c(ifelse(mspecs_optimal$link %in% NA,mspecs_optimal$distance,mspecs_optimal$link),"unmatched"),]
            res <- res[res$restrict %in% c("Restricted"),]
            res$sample <- ifelse(res$sample %in% ifelse(mspecs_optimal$link %in% NA,mspecs_optimal$distance,mspecs_optimal$link),"matched",res$sample)
...
#   dataFrq: A data frame containing the frequency data to be plotted.
#   colset: A vector of colors to be used in the plot. Defaults to c("violet", "purple").
