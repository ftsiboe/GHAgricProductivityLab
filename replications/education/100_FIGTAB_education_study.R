...
# Fig - Robustness              
rm(list= ls()[!(ls() %in% c(Keep.List))])
fig_robustness(y_title="\nLevel difference [Educated less Uneducated]",
               res_list = c(file.path(study_environment$wd$output,"estimations","CropID_Pooled_educated_CD_hnormal_optimal.rds"),
                            list.files(file.path(study_environment$wd$output,"estimations"),
                                       pattern = "CropID_Pooled_educated_TL_",full.names = T)),
               study_environment=study_environment)
...
fig_input_te(
  y_title="\nEducation gap (%)",
  tech_lable=c("Full sample", "Educated sample", "Uneducated sample"),
  study_environment=study_environment)
...
# Fig - Distribution 
rm(list= ls()[!(ls() %in% c(Keep.List))])
dataFrq <- readRDS(file.path(study_environment$wd$output,"estimations/CropID_Pooled_educated_TL_hnormal_fullset.rds"))
