# =============================================================================
#  MATCHING WORKFLOW - FINANCIAL INCLUSION STUDY
# =============================================================================
#  General Description:
#  ---------------------------------------------------------------------------
#  This workflow executes a multi-stage matching and evaluation pipeline for
#  the "FINANCIAL INCLUSION STUDY" project. It performs the following main tasks:
#
...
#         - When executed under SLURM, each array task draws matched samples according to a given
...
DATA$Treat <- as.integer(as.numeric(DATA$credit_hh > 0)) # logical treated flag
...
match_variables_scaler <- c("AgeYr","YerEdu","HHSizeAE","FmleAERt","Depend","CrpMix", crop_area_list,"FinIdxSi")

# Categorical covariates used as factors in matching distance
match_variables_factor <- c("OwnLnd","Ethnic","Marital","Religion","Head")
...
# --- Matching stage 
# Executed if running locally (Windows) or on SLURM jobs named match_all or match_fin
if(grepl("WINDOWS",sysname) || Sys.getenv("SLURM_JOB_NAME") %in% c("match_all", "match_fin")) {
